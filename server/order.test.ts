...
describe("WhatsApp order summary", () => {
  it("formats delivery details and subtotal", () => {
    ...
    expect(result).toContain("Pagamento: Pix — a chave será enviada pelo WhatsApp");
    expect(result).toContain("2x Feijão tropeiro");
    expect(result).toContain("37,00");
  });

  it("formats pickup and optional notes", () => {
    const result = formatOrderMessage({ customerName: "Bia", fulfillment: "pickup", payment: "card_debit", notes: "Sem cebola", lines: [{ name: "Suco", quantity: 1, price: 8 }], subtotal: 8 });
    expect(result).toContain("Retirada na loja");
    expect(result).toContain("Pagamento: cartão de débito na entrega/retirada");
  ...
  it("builds an encoded WhatsApp URL with payment and order details", () => {
    ...
    expect(decodeURIComponent(url)).toContain("Pagamento: Pix — a chave será enviada pelo WhatsApp");
