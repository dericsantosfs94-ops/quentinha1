...
async function nextMenuId(table: "menu_categories" | "menu_products" | "menu_product_options"): Promise<number> {
  const { data, error } = await supabase.from(table).select("id").order("id", { ascending: false }).limit(1).maybeSingle();
  if (error) fail(error, `obter próximo ID de ${table}`);
  return Number(data?.id ?? 0) + 1;
}

export async function createCategory(input: { name: string; slug: string; icon: string }) { const { error } = await supabase.from("menu_categories").insert({ id: await nextMenuId("menu_categories"), name: input.name, slug: input.slug, icon: input.icon }); if (error) fail(error, "criar categoria"); return listAdminMenu(); }
...
export async function createProduct(input: { categoryId: number; name: string; description: string; price: string; imageUrl?: string | null; featuredOfDay: boolean }) { const { error } = await supabase.from("menu_products").insert({ id: await nextMenuId("menu_products"), category_id: input.categoryId, name: input.name, description: input.description, price: input.price, image_url: input.imageUrl ?? null, featured_of_day: input.featuredOfDay }); if (error) fail(error, "criar produto"); return listAdminMenu(); }
...
export async function createProductOption(input: { productId: number; name: string; description?: string | null; priceDelta: string; available: boolean }) { const { error } = await supabase.from("menu_product_options").insert({ id: await nextMenuId("menu_product_options"), product_id: input.productId, name: input.name, description: input.description ?? null, price_delta: input.priceDelta, available: input.available }); if (error) fail(error, "criar opção"); return listAdminMenu(); }
...
export async function clearProductImage(id: number) { const { error } = await supabase.from("menu_products").update({ image_url: null }).eq("id", id); if (error) fail(error, "remover foto do produto"); return listAdminMenu(); }
