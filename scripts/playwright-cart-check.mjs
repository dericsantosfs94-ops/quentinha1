...
try {
  ...
  const card = page.locator("article").filter({ hasText: "Prato da foto 01" }).first();
  await card.getByRole("button", { name: /Adicionar/ }).click();
  await page.getByRole("button", { name: /Ver pedido/ }).click();
  const dialog = page.getByRole("dialog");
  await dialog.getByText("Prato da foto 01").waitFor();
  const image = dialog.locator('img[alt="Prato da foto 01"]');
  ...
  console.log(JSON.stringify({ ok: true, baseUrl, image: imageState, subtotal: "R$ 50,00" }));
} finally {
  await browser.close();
}
