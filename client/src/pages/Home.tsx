...
export default function Home() {
  ...
  if (menuQuery.isLoading) return <div className="flex min-h-screen items-center justify-center bg-[#fff8ef] text-[#8b1e23]"><div className="text-center"><BrandMark /><p className="mt-4 font-display text-2xl">Preparando o cardápio…</p></div></div>;
  if (menuQuery.isError) return <div className="flex min-h-screen flex-col items-center justify-center bg-[#fff8ef] px-5 text-center"><BrandMark /><h1 className="font-display mt-5 text-3xl text-[#8b1e23]">O cardápio não carregou</h1><p className="mt-2 max-w-sm text-[#765e54]">Tente novamente ou fale com a Cantina pelo WhatsApp.</p><button onClick={() => menuQuery.refetch()} className="breathe mt-5 rounded-full bg-[#ff6b35] px-5 py-3 font-extrabold text-[#2b211d]">Tentar novamente</button></div>;
    ...
    <div className="paper-texture min-h-screen pb-24">
      <header className="hero-mesh text-white">
        <div className="mx-auto max-w-6xl px-5 pb-10 pt-5 sm:px-8 sm:pb-14">
          <nav className="flex items-center justify-between" aria-label="Navegação principal">
            <a href="#inicio" className="inline-flex items-center" aria-label="Cantina do Chalé"><BrandMark size="sm" /></a>
      ...
      <main className="mx-auto max-w-6xl px-5 sm:px-8">
        <section id="cardapio" className="pt-12 sm:pt-16">
          ...
          {featuredProducts.length > 0 && <div className="mt-8 rounded-[1.8rem] border border-[#f2b84b]/50 bg-[#fff0dc] p-5"><p className="text-xs font-extrabold uppercase tracking-[.2em] text-[#ff6b35]">Destaque de hoje</p><h3 className="font-display mt-2 text-3xl text-[#8b1e23]">Cardápio do dia</h3><div className="mt-4 flex flex-wrap gap-2">{featuredProducts.map((item) => <button type="button" key={item.id} onClick={() => document.getElementById(`produto-${item.id}`)?.scrollIntoView({ behavior: "smooth", block: "center" })} className="rounded-full border border-[#e3c9b0] bg-white px-4 py-2 text-sm font-bold text-[#8b1e23]">{item.name} · {money.format(item.price)}</button>)}</div></div>}
      ...
      {(cartOpen || checkoutOpen) && <div className="fixed inset-0 z-40 flex items-end justify-center bg-[#2b211d]/50 p-0 sm:items-center sm:p-5" role="dialog" aria-modal="true"><div className="max-h-[92vh] w-full max-w-lg overflow-y-auto rounded-t-[2rem] bg-[#fff8ef] p-5 sm:rounded-[2rem] sm:p-7"><div className="flex items-center justify-between"><div><p className="text-xs font-extrabold uppercase tracking-[.2em] text-[#ff6b35]">{checkoutOpen ? "Quase lá" : "Seu pedido"}</p><h2 className="font-display mt-1 text-3xl text-[#8b1e23]">{checkoutOpen ? "Enviar pelo WhatsApp" : "Carrinho"}</h2></div><button onClick={() => { setCartOpen(false); setCheckoutOpen(false); }} className="flex h-11 w-11 items-center justify-center rounded-full border border-[#ead5c1] bg-white" aria-label="Fechar"><X size={20} /></button></div>
          {!checkoutOpen ? <><div className="mt-6 space-y-3">{!isOpen && <div className="rounded-2xl bg-[#fff0dc] px-4 py-3 text-sm font-bold leading-6 text-[#8b1e23]">A loja está fechada no momento. O checkout ficará disponível quando a Cantina reabrir.</div>}{cart.map((line) => <div key={line.lineId} className="flex items-center gap-3 rounded-2xl border border-[#ead5c1] bg-white p-3"><div className="h-14 w-14 rounded-xl bg-[#f5ddc5]" /><div className="min-w-0 flex-1"><p className="truncate font-extrabold text-[#8b1e23]">{line.name}</p><p className="text-sm text-[#765e54]">{money.format(line.price)}{line.selectedOptions.length > 0 ? ` · ${line.selectedOptions.map((option) => option.name).join(", ")}` : ""}</p></div><div className="flex items-center gap-2"><button onClick={() => changeQuantity(line.lineId, -1)} className="h-9 w-9 rounded-full border border-[#ead5c1] bg-[#fff8ef]" aria-label="Diminuir quantidade"><Minus size={15} className="mx-auto" /></button><span className="w-4 text-center font-bold">{line.quantity}</span><button onClick={() => changeQuantity(line.lineId, 1)} className="h-9 w-9 rounded-full bg-[#ff6b35]" aria-label="Aumentar quantidade"><Plus size={15} className="mx-auto" /></button></div><button onClick={() => setCart((current) => current.filter((item) => item.lineId !== line.lineId))} className="text-[#a83232]" aria-label={`Remover ${line.name}`}><Trash2 size={17} /></button></div>)}</div><div className="mt-6 flex items-center justify-between border-t border-[#ead5c1] pt-4"><span className="font-bold text-[#765e54]">Subtotal</span><strong className="text-xl text-[#8b1e23]">{money.format(subtotal)}</strong></div><button disabled={!isOpen} onClick={() => { if (!isOpen) return; setCartOpen(false); setCheckoutOpen(true); }} className="breathe mt-5 flex min-h-13 w-full items-center justify-center gap-2 rounded-full bg-[#ff6b35] font-extrabold text-[#2b211d] disabled:cursor-not-allowed disabled:opacity-50">Continuar pedido <ArrowRight size={18} /></button></> : <form className="mt-6 space-y-4" onSubmit={(event) => { event.preventDefault(); sendOrder(); }}><label className="block text-sm font-extrabold text-[#2b211d]">Seu nome<input required value={customerName} onChange={(event) => setCustomerName(event.target.value)} className="mt-2 min-h-12 w-full rounded-2xl border border-[#ead5c1] bg-white px-4 outline-none focus:border-[#ff6b35]" placeholder="Como podemos te chamar?" /></label><fieldset><legend className="text-sm font-extrabold text-[#2b211d]">Como prefere receber?</legend><div className="mt-2 grid grid-cols-2 gap-3"><button type="button" onClick={() => setFulfillment("delivery")} className={`min-h-12 rounded-2xl border text-sm font-bold ${fulfillment === "delivery" ? "border-[#ff6b35] bg-[#fff0dc] text-[#8b1e23]" : "border-[#ead5c1] bg-white text-[#765e54]"}`}><Truck size={17} className="mx-auto mb-1" />Entrega</button><button type="button" onClick={() => setFulfillment("pickup")} className={`min-h-12 rounded-2xl border text-sm font-bold ${fulfillment === "pickup" ? "border-[#ff6b35] bg-[#fff0dc] text-[#8b1e23]" : "border-[#ead5c1] bg-white text-[#765e54]"}`}><ShoppingBag size={17} className="mx-auto mb-1" />Retirada</button></div></fieldset><fieldset><legend className="text-sm font-extrabold text-[#2b211d]">Como prefere pagar?</legend><div className="mt-2 grid gap-3 sm:grid-cols-3"><button type="button" onClick={() => setPayment("card_credit")} className={`min-h-12 rounded-2xl border text-sm font-bold ${payment === "card_credit" ? "border-[#ff6b35] bg-[#fff0dc] text-[#8b1e23]" : "border-[#ead5c1] bg-white text-[#765e54]"}`}>Cartão de crédito</button><button type="button" onClick={() => setPayment("card_debit")} className={`min-h-12 rounded-2xl border text-sm font-bold ${payment === "card_debit" ? "border-[#ff6b35] bg-[#fff0dc] text-[#8b1e23]" : "border-[#ead5c1] bg-white text-[#765e54]"}`}>Cartão de débito</button><button type="button" onClick={() => setPayment("pix")} className={`min-h-12 rounded-2xl border text-sm font-bold ${payment === "pix" ? "border-[#ff6b35] bg-[#fff0dc] text-[#8b1e23]" : "border-[#ead5c1] bg-white text-[#765e54]"}`}>Pix via WhatsApp</button></div>{payment === "pix" && <p className="mt-3 rounded-2xl bg-[#fff0dc] px-4 py-3 text-sm leading-6 text-[#765e54]">A Cantina enviará a chave Pix pelo WhatsApp após receber as especificações do pedido.</p>}</fieldset>{fulfillment === "delivery" && <label className="block text-sm font-extrabold text-[#2b211d]">Endereço de entrega<textarea required value={address} onChange={(event) => setAddress(event.target.value)} className="mt-2 min-h-24 w-full rounded-2xl border border-[#ead5c1] bg-white px-4 py-3 outline-none focus:border-[#ff6b35]" placeholder="Rua, número, bairro e referência" /></label>}<label className="block text-sm font-extrabold text-[#2b211d]">Observações <span className="font-normal text-[#765e54]">(opcional)</span><textarea value={notes} onChange={(event) => setNotes(event.target.value)} className="mt-2 min-h-20 w-full rounded-2xl border border-[#ead5c1] bg-white px-4 py-3 outline-none focus:border-[#ff6b35]" placeholder="Algum recado para a Cantina?" /></label><div className="rounded-2xl bg-[#8b1e23] p-4 text-sm text-white"><div className="flex items-center gap-2 font-bold"><CircleAlert size={17} className="text-[#f2b84b]" />Você será levado ao WhatsApp para enviar o pedido.</div></div><button type="submit" disabled={!isOpen} className="breathe flex min-h-13 w-full items-center justify-center gap-2 rounded-full bg-[#ff6b35] font-extrabold text-[#2b211d] disabled:cursor-not-allowed disabled:opacity-50">Enviar pedido no WhatsApp <ArrowRight size={18} /></button></form>}
        </div></div>}
    </div>
  );
}
