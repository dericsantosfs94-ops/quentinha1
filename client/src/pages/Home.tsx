...
export default function Home() {
  ...
  const dialogRef = useDialogFocus(cartOpen || checkoutOpen);
      ...
      <header className="hero-shell relative overflow-hidden text-white">
        ...
        <div className="relative z-10 mx-auto max-w-6xl px-5 pb-10 pt-5 sm:px-8 sm:pb-14">
          <nav className="flex items-center justify-between" aria-label="Navegação principal">
            ...
            <div className="flex items-center gap-3"><Link href="/admin" className="hidden text-xs font-bold text-white/75 underline-offset-4 hover:text-white hover:underline sm:inline">Área administrativa</Link><div className="flex items-center gap-2 text-sm font-bold"><span className={`h-2.5 w-2.5 rounded-full ${isOpen ? "status-pulse bg-[#7fe0a4]" : "bg-[#d4a017] shadow-[0_0_0_4px_rgba(242,184,75,.16)]"}`} />{isOpen ? "Aberto hoje" : "Fechado agora"}</div></div>
          </nav>
          <div id="inicio" className="motion-focal grid items-end gap-8 pt-16 md:grid-cols-[1.15fr_.85fr] md:pt-24">
            <div>
